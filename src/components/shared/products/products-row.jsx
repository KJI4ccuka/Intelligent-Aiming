	...
	return (
		...
		>
			<TableCell className="hidden sm:table-cell p-0">
				<img className="max-w-6 lg:w-12" src={item.icon} alt={`${item.name} icon`} />
			</TableCell>
			
			<TableCell className="text-[7px] leading-3 font-normal p-0 mx-1 md:text-[8.5px] lg:mx-2 lg:text-sm">
				{item.name}
			</TableCell>
			
			<TableCell className="p-0">
				<Badge className="text-[6.6px] h-[18px] py-0 px-1 font-light md:h-full md:px-1.5 lg:text-[11px]" variant="outline">
				...
				<DropdownMenu>
					...
					<DropdownMenuContent className="w-44">
						<Link href="https://intelligent-aiming.xyz/" target="_blank">
							<DropdownMenuItem className="flex text-xs justify-between">
